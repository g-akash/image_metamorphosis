  ...
  public:
    ...
    double lineParameter(Vec2 const & p) const
    {
      // TODO: The formula is in the paper
    	...
    	Vec2 Pfromstart = p-start();
    	double param = (Pfromstart*linevec)/(linevec.length2());
    ...
    double signedLineDistance(Vec2 const & p) const
    {
      // TODO: The formula is in the paper
    	Vec2 linevec = end()-start();
    	Vec2 lineperp = linevec.perp();
    	Vec2 Pfromstart = p-start();
    	double signeddistance = (Pfromstart*lineperp)/linevec.length();
    ...
    double segmentDistance(Vec2 const & p, double u, double v) const
    {
      ...
      {
      	Vec2 Pfromend = p-end();
      	return Pfromend.length();
      }
      else if(u<=0.0)
      {
      	Vec2 Pfromstart = p-start();
      	return Pfromstart.length();
      }
      else
      {
      	return abs(v);
